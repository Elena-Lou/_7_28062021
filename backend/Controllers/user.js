      ...
      if (results.length > 0) {
        ...
        bcrypt
          ...
          .then((encryptedPassword) => {
            ...
            DB.query(
              `INSERT INTO users (email, name, password) VALUES ('${req.body.email}', '${req.body.name}', '${encryptedPassword}')`,
              (error, results, fields) => {
                if (error) {
        ...
        bcrypt
          ...
          .then((valid) => {
            ...
            res.status(200).json({
              userId: results[0].id,
              name: results[0].name,
              token: jwt.sign({ userId: results[0].id, admin: results[0].admin }, process.env.TOKEN, {
                expiresIn: "24h",
              })
            });
          })
